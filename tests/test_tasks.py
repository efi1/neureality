...
def shared_test_logic(cfg_data: ObjectLikeData) -> Tuple[Response, ObjectLikeData]:
    ...
    log_msg = f'calling get {api_path}' if req_type == 'get' else f'calling post {api_path}, data: {object_dump(task_data)}'
    logger.info(F"api-client request: {log_msg}")
    # execute the api call
    response = requests.get(api_path) if req_type == 'get' else requests.post(api_path, json=object_dump(task_data))
    return response, expected_data


def test_perform_tasks(app_container: object, test_name: str, global_dict_data: dict) -> None:
    """
    ...
    """
    cfg_data: data_object = get_param_data(test_name, global_dict_data) # get the test data
